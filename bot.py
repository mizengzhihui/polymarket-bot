...
from config import (
    ...
    SLIPPAGE_TOLERANCE, SCORE_UPDATE_INTERVAL_HOURS, INITIAL_FOLLOW_COUNT,
    ALLOCATION_PAUSE_THRESHOLD, AUTO_DISCOVER,
    POLLING_INTERVAL_SEC, CLOSE_MONITOR_INTERVAL_SEC,
    safe_get, GAMMA_API,
)
from leaderboard import (
    get_leaderboard, get_trader_profile, get_trader_value,
    get_trader_trades, get_trader_positions, get_market_info,
    get_all_leaderboards, scan_newbies,
)
from trader import calculate_copy_size, place_copy_order, get_own_positions, \
    ...
    get_order_book, poll_all_trader_buys, poll_all_trader_sells
...
_position_peak_pnl = {}  # asset -> peak cash_pnl (for trailing stop)

# Auto-discovery state
_discovered_wallets: list[str] = []  # wallets found by auto-discovery
_discovery_retry_count = 0
_last_discovery_ts = 0
_available_capital = 0.0
_used_capital = 0.0
...
def _run_perf_review(score_engine):
    ...
    # After review, run auto-rotation
    _auto_rotate_traders(score_engine)
_condition_cache_ts = 0
_last_rotation_ts = 0  # Rate limit auto-rotation (max 1 per 7 days)
_ROTATION_MIN_DAYS = 7  # Min days between rotations


def _auto_rotate_traders(score_engine):
    """Self-evolving trader rotation: drop worst, add best candidate.
    Only rotates if the worst trader has negative realized PnL AND
    the best candidate has a significantly higher score.
    Rate limited to once per ROTATION_MIN_DAYS."""
    global _last_rotation_ts, FOLLOW_WALLETS

    if int(time.time()) - _last_rotation_ts < _ROTATION_MIN_DAYS * 86400:
        return

    if len(FOLLOW_WALLETS) < 1:
        return

    # Find worst followed trader by realized PnL
    worst_wallet = None
    worst_pnl = float('inf')
    for w in FOLLOW_WALLETS:
        pd = _trader_pnl.get(w, {})
        rp = pd.get("realized_pnl", 0)
        trades = pd.get("trades", 0)
        if trades >= 3 and rp < worst_pnl:
            worst_pnl = rp
            worst_wallet = w

    if not worst_wallet or worst_pnl >= 0:
        return  # No bad performer to drop

    # Find best candidate among scored but not followed
    followed_set = {w.lower() for w in FOLLOW_WALLETS}
    best_candidate = None
    best_score = 0
    for w, s in score_engine._scores.items():
        if w.lower() in followed_set:
            continue
        if s.get("reliable") and s.get("score", 0) > best_score:
            best_score = s["score"]
            best_candidate = w

    worst_score = score_engine.get_score(worst_wallet).get("score", 0)

    # Only rotate if candidate is significantly better (+15 pts)
    if not best_candidate or best_score < worst_score + 15:
        return

    # Execute rotation
    logger.log("check", f"ROTATION: dropping {_trader_short(worst_wallet)} "
               f"(PnL ${worst_pnl:+.2f}, score {worst_score}) → "
               f"adding {_trader_short(best_candidate)} (score {best_score})")

    FOLLOW_WALLETS.remove(worst_wallet)
    FOLLOW_WALLETS.append(best_candidate)
    score_engine.release_allocation(worst_wallet, 0)  # Clear allocation
    _last_rotation_ts = int(time.time())

    send_feishu(
        "🔄 交易员轮换 — 自进化优化",
        [f"**移除:** {_trader_short(worst_wallet)}",
         f"  累计盈亏: ${worst_pnl:+.2f}　|　评分: {worst_score}",
         f"**加入:** {_trader_short(best_candidate)}",
         f"  评分: {best_score}　|　{score_engine.get_score(best_candidate).get('days', 0)}天数据",
         f"",
         f"**当前跟单:** {len(FOLLOW_WALLETS)} 人"],
        color="blue",
    )
...
# ============================================================
# Auto-Discovery (v1.3 merged)
# ============================================================
def discover_traders(score_engine: TraderScoreEngine) -> list[str]:
    """Fetch leaderboards + newbie scan, score, select top N.
    Returns list of wallet addresses to follow."""
    log_info = print
    candidates = get_all_leaderboards(top_n=20)
    log_info(f"[Discovery] Leaderboard candidates: {len(candidates)}")

    newbies = scan_newbies()
    log_info(f"[Discovery] Newbie candidates: {len(newbies)}")
    ...
    log_info(f"[Discovery] Candidate pool: {len(pool)} traders")
    ...
    selected = [s[0]["address"] for s in scored[:n]]
    log_info(f"[Discovery] Selected top {len(selected)} traders")
    return selected


def check_slippage(token_id: str, side: str, target_price: float) -> bool:
    ...
    try:
        ...
        if book:
            ...
            if entries:
                entry = entries[-1]
                best = float(entry.get("price", 0)) if isinstance(entry, dict) else float(entry[0])
    except Exception:
        pass

    if best is None:
        print(f"  No orderbook for {token_id[:10]}..., skipping")
        return False
    if target_price <= 0:
        return False
    slippage = abs(best - target_price) / target_price
    if slippage > SLIPPAGE_TOLERANCE:
        print(f"  Slippage {slippage*100:.1f}% > {SLIPPAGE_TOLERANCE*100:.0f}%, skipping")
        return False
    return True


def send_2h_report(trader_pnl: dict, own_positions: list, dry_run: bool):
    """Send 2-hour status report (Beijing time aligned)."""
    try:
        own_p = own_positions or get_own_positions()
        bal = get_own_balance()
...
def run_bot(dry_run=True, poll_interval=5):
    ...
    last_2h_report = last_check  # 2-hour timed report (Beijing time aligned)
    last_network_ok = last_check  # network health check
    last_discovery = last_check  # auto-discovery cycle
    last_close_check = last_check  # dedicated close monitoring
    last_emergency_check = last_check  # emergency block check (every 5 min)
    last_own_pos = 0  # own position cache TTL
    ...
    # Seed lb_pnl cache from existing snapshots (for emergency block portfolio estimation)
    ...
    # Auto-discovery: if FOLLOW_WALLETS is empty or AUTO_DISCOVER is set,
    # discover traders from leaderboards
    global _discovered_wallets, _discovery_retry_count, _last_discovery_ts
    global _available_capital, _used_capital
    _available_capital = USER_CAPITAL
    _used_capital = 0.0

    effective_wallets = list(FOLLOW_WALLETS)
    if AUTO_DISCOVER or not effective_wallets:
        print("[Discovery] Auto-discovery enabled, scanning leaderboards...")
        try:
            _discovered_wallets = discover_traders(score_engine)
            _last_discovery_ts = int(time.time())
            for w in _discovered_wallets:
                if w not in effective_wallets:
                    effective_wallets.append(w)
            print(f"[Discovery] Added {len(_discovered_wallets)} discovered traders")
            send_feishu("自动发现交易员", [
                f"发现 {len(_discovered_wallets)} 个交易员",
                f"有效跟单总数: {len(effective_wallets)}",
            ])
        except Exception as e:
            print(f"[Discovery] Initial discovery failed: {e}")

    if not effective_wallets:
        print("ERROR: No traders to follow (manual list empty + discovery failed).")
        print("  Use --show-leaderboard to browse top traders first.")
        return

    # Use effective_wallets for all trading operations
    FOLLOW_WALLETS[:] = effective_wallets
    ...
    while True:
        try:
            ...
            # Update own positions (cached: refresh every 30s)
            _now = int(time.time())
            if _now - last_own_pos > 30:
                own_pos = get_own_positions()
            ...
            for p in own_pos:
                ...
                if pnl < -pos_loss_limit and asset and asset not in _sl_triggered:
                    ...
                    if not dry_run:
                        ...
                        if ok:
                            owner = _trader_asset_owner.get(asset, "")
                            if owner:
                                score_engine.release_allocation(owner, size * price)
                            send_feishu(
                                "🛑 止损平仓",
            ...
            for p in own_pos:
                ...
                # Trailing take-profit: once PnL > 20%, track peak, exit if drops 15%
                if pnl > 0 and pnl_pct > 0.20:
                    peak = _position_peak_pnl.get(asset, 0)
                    if pnl > peak:
                        _position_peak_pnl[asset] = pnl
                        peak = pnl
                    peak_pct = peak / cost_basis
                    drawdown_from_peak = (peak - pnl) / peak if peak > 0 else 0
                    if drawdown_from_peak > 0.15:
                        size = p.get("size", 0)
                        price = p.get("current_price", 0)
                        title = p.get("title", "")[:30]
                        print(f"  TRAIL-STOP: {title} +{pct_display:.0f}% (peak +{peak_pct*100:.0f}%, dd {drawdown_from_peak*100:.0f}%), selling {size:.0f} sh")
                        logger.log("trailing_stop", f"{title} peak +{peak_pct*100:.0f}% dd {drawdown_from_peak*100:.0f}% selling {size:.0f} sh")
                        ...
                        if not dry_run:
                            ...
                            if ok:
                                ...
                                if owner:
                                    score_engine.release_allocation(owner, size * price)
                                logger.add_trade("TRAIL_STOP", title, "SELL", size * price, oid)
                                send_feishu(
                                    f"📈 移动止盈 (+{pct_display:.0f}%, 峰值+{peak_pct*100:.0f}%)",
                                    [f"**市场:** {title}",
                                     f"**盈利:** +${pnl:.2f} (+{pct_display:.0f}%)",
                                     f"**峰值:** +${peak:.2f} (+{peak_pct*100:.0f}%)",
                                     f"**卖出:** {size:.0f} 股 @ ${price:.4f}"],
                                    color="green",
                                )
                        else:
                            logger.log("trailing_stop", f"[DRY] trail-stop {title} {size:.0f} sh")
                        # Clean peak tracker
                        _position_peak_pnl.pop(asset, None)
                        continue

                # Fixed take-profit: cash_pnl > TAKE_PROFIT_PCT * cost
                ...
                if pnl < 0 and pnl_pct > STOP_LOSS_PCT and asset not in _sl_triggered:
                    _sl_triggered.add(asset)
                    size = p.get("size", 0)
                    price = p.get("current_price", 0)
                    title = p.get("title", "")[:30]
            ...
            # Clean stale _sl_triggered entries (position no longer held)
            held_assets = {p.get("asset", "") for p in own_pos}
            _sl_triggered &= held_assets
            ...
            for wallet in FOLLOW_WALLETS:
                ...
                # Runs every 5 min (not every cycle) to avoid API waste
                if now_ts - last_emergency_check > 300:
                    if not score_engine.is_blocked(wallet):
                        try:
                            ...
                            if ref <= 0:
                                ref = total_initial or max(USER_CAPITAL * 2, 500)
                ...
                for trade in trades:
                        ...
                        if matching and matching.get("size", 0) > 0:
                            ...
                            if not dry_run:
                                ...
                                if ok:
                                    ...
                                    score_engine.release_allocation(wallet, sell_usd)
                    ...
                    # Signal freshness: skip trades older than 90s
                    trade_ts = trade.get("timestamp", 0)
                    if trade_ts and (int(time.time()) - trade_ts) > 90:
                        print(f"  STALE: {ts} trade {int(time.time())-trade_ts}s old, skipping")
                        continue
                    ...
                    # Dynamic PnL multiplier: boost/dampen based on actual copy results
                    pnl_data = _trader_pnl.get(wallet, {})
                    total_buys = pnl_data.get("buys_usd", 0)
                    realized = pnl_data.get("realized_pnl", 0)
                    if total_buys > 0:
                        pnl_ratio = realized / (total_buys + 1)
                        pnl_mult = max(0.2, min(2.0, 1.0 + pnl_ratio * 2))
                        if abs(pnl_mult - 1.0) > 0.01:
                            copy_usd *= pnl_mult
                            print(f"  PNL-MULT: x{pnl_mult:.2f} (PnL ${realized:+.2f}/{total_buys:.0f})")
                    ...
                    # Cascade allocation cap (score-weighted, per-trader)
                    all_wallets = list(FOLLOW_WALLETS)
                    cascade_cap = score_engine.compute_allocation(
                        wallet, USER_CAPITAL - sum(p.get("current_value", 0)
                                                    for p in own_pos), all_wallets)
                    if cascade_cap > 0 and copy_usd > cascade_cap:
                        print(f"  CASCADE: reducing ${copy_usd:.1f}→${cascade_cap:.1f} (per-wallet cap)")
                        copy_usd = cascade_cap
                    if cascade_cap <= 0 and score_engine.get_score(wallet).get("blocked"):
                        print(f"  SKIP: trader {_trader_short(wallet)} is blocked")
                        continue
                    ...
                    if dry_run:
                        ...
                        if trade_side == "BUY":
                            ...
                            if balance is not None and copy_usd > balance * 0.95:
                                ...
                                continue
                        # Slippage check: skip if market moved too far
                        if not check_slippage(trade_asset, trade_side, trade_price):
                            continue
                        ...
                        if success:
                            ...
                            score_engine.record_allocation_used(wallet, copy_usd)
            ...
            # ---- 24h auto-discovery cycle ----
            if AUTO_DISCOVER and now_ts - last_discovery > SCORE_UPDATE_INTERVAL_HOURS * 3600:
                print("[Cycle] Running trader discovery...")
                score_engine.reset_cycle()
                try:
                    _discovered_wallets = discover_traders(score_engine)
                    _last_discovery_ts = now_ts
                    # Merge with manual FOLLOW_WALLETS
                    merged = list(FOLLOW_WALLETS)
                    for w in _discovered_wallets:
                        if w not in merged:
                            merged.append(w)
                    FOLLOW_WALLETS[:] = merged
                    print(f"[Cycle] Discovery: {len(_discovered_wallets)} discovered, "
                          f"{len(FOLLOW_WALLETS)} total")
                    send_feishu("跟单列表更新",
                                [f"发现 {len(_discovered_wallets)} 个新交易员",
                                 f"总跟单: {len(FOLLOW_WALLETS)}"])
                    _discovery_retry_count = 0
                except Exception as e:
                    print(f"[Cycle] Discovery error: {e}")
                last_discovery = now_ts

            # ---- Close monitoring (dedicated poll cycle, backup for main loop) ----
            if now_ts - last_close_check > CLOSE_MONITOR_INTERVAL_SEC:
                try:
                    sells = poll_all_trader_sells(FOLLOW_WALLETS)
                    for wallet, trades in sells.items():
                        for t in trades:
                            trade_asset = t.get("asset", "")
                            ...
                            for p in own_pos:
                                if p.get("asset") == trade_asset:
                                    ...
                                    break
                            if matching and matching.get("size", 0) > 0:
                                sell_shares = matching["size"]
                                print(f"  CLOSE: {_trader_short(wallet)} sold "
                                      f"{trade_asset[:12]}...")
                                if not dry_run:
                                    ok, oid, err = place_copy_market_order(
                                        trade_asset, "SELL", sell_shares, trade_price)
                                    if ok:
                                        logger.add_trade(_trader_short(wallet),
                                                         trade_asset[:30], "SELL",
                                                         sell_shares * trade_price, oid)
                                        score_engine.release_allocation(wallet,
                                                                        sell_shares * trade_price)
                                        send_feishu("📤 跟单平仓", [
                                            f"**交易者:** {_trader_short(wallet)}",
                                            f"**资产:** {trade_asset[:20]}...",
                                            f"**卖出:** {sell_shares:.0f} 股",
                                        ])
                                    else:
                                        logger.add_error(f"Close failed: {(err or '')[:80]}")
                except Exception:
                    pass
                last_close_check = now_ts

            # ---- 2-hour timed report (Beijing time aligned) ----
            now_dt2 = datetime.datetime.now(datetime.timezone.utc)
            beijing_hour = (now_dt2.hour + 8) % 24
            if beijing_hour % 2 == 0 and now_ts - last_2h_report > 3600:
                if now_dt2.minute < 5:
                    try:
                        send_2h_report(_trader_pnl, own_pos, dry_run)
                    except Exception:
                        pass
                    last_2h_report = now_ts

            # ---- Network health check (every 300s) ----
            if now_ts - last_network_ok > 300:
                try:
                    import urllib.request as _ur
                    _ur.urlopen("https://clob.polymarket.com/book?token_id=1",
                                timeout=5)
                    last_network_ok = now_ts
                except Exception:
                    pass

            # ---- Capital pause check ----
            total_open_usd = sum(p.get("current_value", 0) for p in own_pos)
            used_pct = total_open_usd / USER_CAPITAL if USER_CAPITAL > 0 else 0
            if used_pct >= ALLOCATION_PAUSE_THRESHOLD and not logger.circuit_breaker:
                print(f"  CAP: {used_pct:.0%} allocated, pausing new entries")
                # Don't set circuit breaker, just skip new buys naturally
