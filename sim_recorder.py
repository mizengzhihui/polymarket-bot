...
from config import USER_CAPITAL
...
SIM_MAX_POSITION_SIZE_CAP = 50  # 单笔绝对上限
...
def generate_monthly_report() -> str:
    ...
    real_total_usd = sum(r["real"]["copy_usd"] for r in records
                         if r["real"]["action"] == "BUY")

    levels_stats = {}
    for cap in SIM_CAPITAL_LEVELS:
        sk = str(cap)
        buys = sum(1 for r in records
        ...
        levels_stats[cap] = {"buys": buys, "skips": skips, "total_usd": total_usd}
    ...
    lines = [
        ...
        "|---------|---------|-------|------------|-------|",
        f"| **${int(USER_CAPITAL)} (实盘)** | {real_buys} | {real_skips} | "
        f"${real_total_usd:.2f} | {real_buys/total*100:.1f}% |",
    ]

    for cap in SIM_CAPITAL_LEVELS:
        s = levels_stats[cap]
        lines.append(
            f"| **${cap}** | {s['buys']} | {s['skips']} | "
            f"${s['total_usd']:.2f} | {s['buys']/total*100:.1f}% |"
        )
